...
namespace HoloToolkit.Unity
{
    ...
    public class HeadsUpDirectionIndicatorX : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            m_color = PointerMaterial.GetColor("_Color");
            //m_follow.onStateChanged += M_follow_onStateChanged;
            m_transition = 0f;
            //m_originalTransparency = m_color.a;
        }

        private bool m_pointerIsActive = false;

        private void M_follow_onStateChanged(bool state)
        {
            m_pointerIsActive = !state;  
        }
        ...
        private List<string> m_visibleObjects = new List<string>();
        private float m_lastVisibilityUpdate;
        [SerializeField]
        private float m_originalTransparency;
        private Color m_color;

        public void OnTargetBecameVisible(string id)
        {
            if (!m_visibleObjects.Contains(id))
            {
                m_visibleObjects.Add(id);
            }
            m_lastVisibilityUpdate = Time.time;
        }

        public void OnTargetBecameInvisible(string id)
        {
            if (m_visibleObjects.Contains(id))
            {
                m_visibleObjects.Remove(id);
            }
            m_lastVisibilityUpdate = Time.time;
        }
        ...
        private void CalcTransparency()
        {
            ...
            {
                m_transition -= Time.deltaTime / m_transitionTime;
                //Debug.Log("Decr");
            }
            else
            {
            ...
            m_transition = Mathf.Min(Mathf.Max(m_transition, 0f), 1f);
            if (m_transition <= 0f)
            {
            ...
            //Mathf.Min((Time.time - m_lastVisibilityUpdate) / 2, 1)
            m_color.a =  m_transition * m_originalTransparency;
            PointerMaterial.SetColor("_Color", m_color);
        }
        [SerializeField]
        private FollowGazePoint m_follow;
    }
