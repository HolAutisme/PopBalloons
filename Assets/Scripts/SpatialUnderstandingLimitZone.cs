...
namespace HoloToolkit.Unity
{
    ...
    /// </summary>
    public class SpatialUnderstandingLimitZone : SpatialMappingSource
    {
        // Config
        ...
        private float lowestMeshPoint = 0f;
        ...
        private class MeshData
        {
            ...
            public void AddFloorSurfaceTriangle(Vector3 point1, Vector3 point2, Vector3 point3)
            {
        ...
        private void AddTriangleToSector(Vector3 sector, Vector3 point1, Vector3 point2, Vector3 point3)
        {
            ...
            float[] listPointX = new[] { point1.x, point2.x, point3.x };
            float[] listPointY = new[] { point1.y, point2.y, point3.y };
            float[] listPointZ = new[] { point1.z, point2.z, point3.z };
            setLowestMeshPoints(listPointX, listPointY, listPointZ);

            point1.y = lowestMeshPoint;
            point2.y = lowestMeshPoint;
            point3.y = lowestMeshPoint;
            // Add the vertices to the sector's mesh container.
            nextSectorData.AddFloorSurfaceTriangle(point1, point2, point3);
        }

        private void setLowestMeshPoints(float[] listPointX, float[] listPointY, float[] listPointZ)
        {
            if (listPointY.Min() < lowestMeshPoint)
                lowestMeshPoint = listPointY.Min();

            if (listPointX.Min() < xMin)
                xMin = listPointX.Min();
            if (listPointZ.Min() < zMin)
                zMin = listPointZ.Min();
            if (listPointX.Max() > xMax)
                xMax = listPointX.Max();
            if (listPointZ.Max() > zMax)
                zMax = listPointZ.Max();
            corners = new Vector3[] { new Vector3(xMin, 0, zMin), new Vector3(xMax, 0, zMin), new Vector3(xMax, 0, zMax), new Vector3(xMin, 0, zMax) };
        }

        public Vector3[] geCorners()
        {
            return corners;
        }

        public float getLowestMeshPoint()
        {
            return lowestMeshPoint;
        }
